...
class Deep3Dnet:
    ...
    def build(self, rgb, train_mode=None):
        ...
        """
        get_act = 0
        get_act += reduce(lambda x, y: x*y, rgb.get_shape().as_list())
        rgb_scaled = rgb * 255.0
        get_act += reduce(lambda x, y: x*y, rgb_scaled.get_shape().as_list())
        # Convert RGB to BGR
        ...
        get_act += reduce(lambda x, y: x*y, self.conv1_1.get_shape().as_list())
        
        self.conv1_2 = self.conv_layer(self.conv1_1, 64, 64, "conv1_2")
        get_act += reduce(lambda x, y: x*y, self.conv1_2.get_shape().as_list())
        self.pool1 = self.max_pool(self.conv1_2, 'pool1')
        get_act += reduce(lambda x, y: x*y, self.pool1.get_shape().as_list())

        self.conv2_1 = self.conv_layer(self.pool1, 64, 128, "conv2_1")
        get_act += reduce(lambda x, y: x*y, self.conv2_1.get_shape().as_list())
        self.conv2_2 = self.conv_layer(self.conv2_1, 128, 128, "conv2_2")
        get_act += reduce(lambda x, y: x*y, self.conv2_2.get_shape().as_list())
        self.pool2 = self.max_pool(self.conv2_2, 'pool2')
        get_act += reduce(lambda x, y: x*y, self.pool2.get_shape().as_list())

        self.conv3_1 = self.conv_layer(self.pool2, 128, 256, "conv3_1")
        get_act += reduce(lambda x, y: x*y, self.conv3_1.get_shape().as_list())
        self.conv3_2 = self.conv_layer(self.conv3_1, 256, 256, "conv3_2")
        get_act += reduce(lambda x, y: x*y, self.conv3_2.get_shape().as_list())
        self.conv3_3 = self.conv_layer(self.conv3_2, 256, 256, "conv3_3")
        get_act += reduce(lambda x, y: x*y, self.conv3_3.get_shape().as_list())
        self.conv3_4 = self.conv_layer(self.conv3_3, 256, 256, "conv3_4")
        get_act += reduce(lambda x, y: x*y, self.conv3_4.get_shape().as_list())
        self.pool3 = self.max_pool(self.conv3_4, 'pool3')
        get_act += reduce(lambda x, y: x*y, self.pool3.get_shape().as_list())

        self.conv4_1 = self.conv_layer(self.pool3, 256, 512, "conv4_1")
        get_act += reduce(lambda x, y: x*y, self.conv4_1.get_shape().as_list())
        self.conv4_2 = self.conv_layer(self.conv4_1, 512, 512, "conv4_2")
        get_act += reduce(lambda x, y: x*y, self.conv4_2.get_shape().as_list())
        self.conv4_3 = self.conv_layer(self.conv4_2, 512, 512, "conv4_3")
        get_act += reduce(lambda x, y: x*y, self.conv4_3.get_shape().as_list())
        self.conv4_4 = self.conv_layer(self.conv4_3, 512, 512, "conv4_4")
        get_act += reduce(lambda x, y: x*y, self.conv4_4.get_shape().as_list())
        self.pool4 = self.max_pool(self.conv4_4, 'pool4')
        get_act += reduce(lambda x, y: x*y, self.pool4.get_shape().as_list())

        self.conv5_1 = self.conv_layer(self.pool4, 512, 512, "conv5_1")
        get_act += reduce(lambda x, y: x*y, self.conv5_1.get_shape().as_list())
        self.conv5_2 = self.conv_layer(self.conv5_1, 512, 512, "conv5_2")
        get_act += reduce(lambda x, y: x*y, self.conv5_2.get_shape().as_list())
        self.conv5_3 = self.conv_layer(self.conv5_2, 512, 512, "conv5_3")
        get_act += reduce(lambda x, y: x*y, self.conv5_3.get_shape().as_list())
        self.conv5_4 = self.conv_layer(self.conv5_3, 512, 512, "conv5_4")
        get_act += reduce(lambda x, y: x*y, self.conv5_4.get_shape().as_list())
        self.pool5 = self.max_pool(self.conv5_4, 'pool5')
        get_act += reduce(lambda x, y: x*y, self.pool5.get_shape().as_list())

        
        print 'activation checkpoint 1'
        print get_act

        self.fc6 = self.fc_layer(self.pool5, 23040, 4096, "fc6") #23040=((160//(2**5))*(288//(2**5)))*512
        get_act += reduce(lambda x, y: x*y, self.fc6.get_shape().as_list())
        self.relu6 = tf.nn.relu(self.fc6)
        get_act += reduce(lambda x, y: x*y, self.relu6.get_shape().as_list())
        if train_mode is not None and self.trainable:
            self.relu6 = tf.nn.dropout(self.relu6, self.dropout)

        self.fc7 = self.fc_layer(self.relu6, 4096, 4096, "fc7")
        get_act += reduce(lambda x, y: x*y, self.fc7.get_shape().as_list())
        self.relu7 = tf.nn.relu(self.fc7)
        get_act += reduce(lambda x, y: x*y, self.relu7.get_shape().as_list())
        ...
        get_act += reduce(lambda x, y: x*y, self.fc8.get_shape().as_list())
       
    
        print 'activation checkpoint 2'
        print get_act
        
        scale = 16
        #this is is resizing the output of the fully connected layers
        self.pred5_1 = tf.reshape(self.fc8,[-1,5,9,33])
        get_act += reduce(lambda x, y: x*y, self.pred5_1.get_shape().as_list())
        self.pred5_2 = tf.nn.relu(self.pred5_1)
        get_act += reduce(lambda x, y: x*y, self.pred5_2.get_shape().as_list())
        self.pred5_3 = self.deconv_layer(self.pred5_2,33,33,scale,0,'pred5_deconv')
        get_act += reduce(lambda x, y: x*y, self.pred5_3.get_shape().as_list())

        self.feat_act = tf.nn.relu(self.pred5_3)
        get_act += reduce(lambda x, y: x*y, self.feat_act.get_shape().as_list())

        scale = 2
        self.up_1 = self.deconv_layer(self.feat_act,33,33,scale,0,'up_deconv')
        get_act += reduce(lambda x, y: x*y, self.up_1.get_shape().as_list())
        self.up_2 = tf.nn.relu(self.up_1)
        get_act += reduce(lambda x, y: x*y, self.up_2.get_shape().as_list())
        self.up_3 = self.conv_layer(self.up_2, 33, 33, "up_conv")
        get_act += reduce(lambda x, y: x*y, self.up_3.get_shape().as_list())
        
        self.mask = tf.nn.softmax(self.up_3)
        get_act += reduce(lambda x, y: x*y, self.mask.get_shape().as_list())
        self.prob  = selection.select(self.mask, rgb)
        get_act += reduce(lambda x, y: x*y, self.prob.get_shape().as_list())
        
        print 'activation checkpoint 3'
        print get_act
